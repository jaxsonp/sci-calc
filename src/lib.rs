...
fn evaluate_ast(root: Expr) -> Result<f64, CalcError> {
	match root {
		Expr::Num(n) => {
			Ok(n)
		}
		Expr::Op(left_e, op, right_e) => {
			...
			let lhs = match evaluate_ast(*left_e) {
				Ok(n) => n,
				Err(e) => { return Err(e) },
			};
			let rhs = match evaluate_ast(*right_e) {
				Ok(n) => n,
				Err(e) => { return Err(e) },
			};
		...
		Expr::Fac(e) => {
			// TODO fix :(
			let mut n = match evaluate_ast(*e) {
				Ok(n) => n,
				Err(e) => { return Err(e) },
			};
			let mut res = 1f64;
